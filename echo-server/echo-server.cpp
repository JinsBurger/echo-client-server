...
vector<unsigned int> cfds;
pthread_mutex_t fdvec_mutex = PTHREAD_MUTEX_INITIALIZER;;
...
void recvThread(int sd) {
	...
	while (true) {
		...
		fflush(stdout);
		if(param.broadcast) {
			for(unsigned int cfd: cfds) {
				res = ::send(cfd, buf, res, 0);
				if (res == 0 || res == -1) {
	...
	pthread_mutex_lock(&fdvec_mutex);
	cfds.erase(find(cfds.begin(), cfds.end(), sd));
	::close(sd);
	pthread_mutex_unlock(&fdvec_mutex);
}
